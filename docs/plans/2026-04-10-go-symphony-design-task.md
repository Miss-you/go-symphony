...
| T18 | CLI + Full Parity Test Sweep | Recreate CLI behavior, startup acknowledgement, shutdown rendering, and full parity-oriented verification. | T16, T17 | P13 | done | Codex | 2026-04-12 09:37 CST | workspace/T18 | cli-full-parity-test-sweep | CLI behavior matches the design and the full parity test matrix passes or is explicitly documented. | Isolated worktree: `.worktrees/t18-cli-full-parity-test-sweep`; review findings recorded in `workspace/T18/code_review.md`; final comparison in `workspace/T18/final_compare.md`; residual live-provider notes in `workspace/T18/todo.md`; change archived at `openspec/changes/archive/2026-04-12-cli-full-parity-test-sweep/`; final validation passed (`make verify`, `openspec validate --specs`, `git diff --check`). |
| T19 | Verification Workflows | Add operator-facing verification helpers for isolated Linear data probing and controlled runtime/Codex end-to-end smoke runs. | T18 | P14 | done | Codex | 2026-04-15 13:31 CST | workspace/T19 | verification-workflows | Operators can validate Linear reader behavior without starting workers, then run a guarded single-item runtime/Codex smoke path with documented workflow templates. | Isolated worktree: `.worktrees/t19-verification-workflows`; review findings recorded in `workspace/T19/code_review.md` and `workspace/T19/code_review_followup.md`; final comparison in `workspace/T19/final_compare.md`; residual live-provider notes in `workspace/T19/todo.md`; change archived at `openspec/changes/archive/2026-04-15-verification-workflows/`; final validation passed (`make verify`, `openspec validate --specs`, `git diff --check`). |
...
- 2026-04-12 09:37 CST: Claimed `T18` for end-to-end delivery in isolated worktree `.worktrees/t18-cli-full-parity-test-sweep` after a passing baseline (`go test ./...`). Owner=`Codex`, workspace=`workspace/T18`.
- 2026-04-12 09:37 CST: Created `workspace/T18/` in the isolated worktree and moved `T18` to `research`.
- 2026-04-12 09:45 CST: Accepted `workspace/T18/final_impl.md` after rubric review (reviews 91/84, avg 87.5, no high-severity issues) and moved `T18` to `spec`.
- 2026-04-12 09:45 CST: Created and validated OpenSpec change `cli-full-parity-test-sweep` for `T18` and linked it in the task board.
- 2026-04-12 09:45 CST: Wrote `workspace/T18/test_strategy.md`, cleared spec review with no high-severity issues, and moved `T18` to `implementing`.
- 2026-04-12 09:45 CST: Implemented CLI acknowledgement, argument parsing, log-root setup, port override, offline shutdown rendering, and no-network e2e smoke coverage; moved `T18` to `verifying`.
- 2026-04-12 09:45 CST: Fresh verification passed (`go test -count=1 ./internal/cli/... ./cmd/symphony/...`, dashboard/API/observability package gate, `go test -count=1 ./...`, `make build`, `make lint`, `make test`, `make test-e2e`, `make verify`, `openspec validate --type change cli-full-parity-test-sweep`, `openspec validate --specs`, `git diff --check`) and moved `T18` to `review`.
- 2026-04-12 09:45 CST: Code review found one e2e coverage gap and one offline-render assertion gap; both were fixed with targeted tests and fresh package/e2e verification.
- 2026-04-12 09:45 CST: Final comparison and residual notes were recorded in `workspace/T18/`.
- 2026-04-12 09:45 CST: Synced `cli-bootstrap-parity` into main specs, archived `cli-full-parity-test-sweep` to `openspec/changes/archive/2026-04-12-cli-full-parity-test-sweep/`, final validation passed (`make verify`, `openspec validate --specs`, `git diff --check`), and marked `T18` done.
- 2026-04-15 13:31 CST: Added and claimed `T19` for verification workflow helpers in isolated worktree `.worktrees/t19-verification-workflows` after baseline `go test ./...` passed. Owner=`Codex`, workspace=`workspace/T19`.
- 2026-04-15 13:31 CST: Created `workspace/T19/` in the isolated worktree and moved `T19` to `research`.
- 2026-04-15 13:31 CST: Accepted `workspace/T19/final_impl.md` after round-two rubric review (reviews 92/88, avg 90/100, no high-severity issues) and moved `T19` to `spec`.
- 2026-04-15 13:31 CST: Created and validated OpenSpec change `verification-workflows`, wrote `workspace/T19/test_strategy.md`, passed spec review with no high-severity issues, and moved `T19` to `implementing`.
- 2026-04-15 13:31 CST: Implemented verification helpers and docs with TDD coverage, moved `T19` to `verifying`, and fresh verification passed (`go test ./internal/tracker/... ./cmd/symphony-verify/...`, `go test ./...`, `make build`, `make test-e2e`, `make verify`, `openspec validate --type change verification-workflows`, `openspec validate --specs`, `git diff --check`).
- 2026-04-15 13:31 CST: Entered `review` for `T19` after verification evidence was recorded in `workspace/T19/verification.md`.
- 2026-04-15 13:31 CST: Code review follow-up split the pure Linear probe into `internal/verify/linearprobe`, strengthened the dependency-boundary test, and fixed the verification workflow template.
- 2026-04-15 13:31 CST: Final comparison and residual live-provider notes were recorded in `workspace/T19/`.
- 2026-04-15 13:31 CST: Synced `verification-workflows` into main specs, archived the change to `openspec/changes/archive/2026-04-15-verification-workflows/`, final validation passed (`make verify`, `openspec validate --specs`, `git diff --check`), and moved `T19` to `done`.
- 2026-04-15 13:31 CST: Follow-up live acceptance using local `t19.env` validated Linear read probe, comment creation, no-op state update, and `Backlog -> In Progress -> In Review -> Done` state transition for the disposable `YOU-21` issue; troubleshooting notes were recorded in `docs/verification-workflows.md` and `workspace/T19/verification.md`.
- 2026-04-15 16:25 CST: Reclassified the earlier operator-driven `YOU-21`/`YOU-22` state mutations and `go-symphony` PR as invalid product acceptance evidence; added `workspace/T19/reverification_sop.md`, had it independently reviewed, tightened repo/source attribution rules, added `linear --only-issue`, and completed strict `YOU-22` live validation through go-symphony runtime hooks against `Miss-you/test-go-symphony` PR #1. Evidence recorded in `workspace/T19/verification.md`.
