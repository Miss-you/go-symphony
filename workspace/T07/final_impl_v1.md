...
This plan is intended to stay narrow, Go-native, and parity-first. It preserves the workspace lifecycle semantics that are visible in the Elixir implementation, but it does not freeze runner, Codex, or tracker-write behavior early.
...
- a narrow lifecycle manager or helper set that takes the already-normalized config values from `internal/config`
- explicit methods for create, pre-run hook, post-run hook, and removal
- a path helper that derives a workspace path from the configured root and a normalized identifier
- internal seams for executing hook commands and removing directories, so tests can control timeout and failure behavior without baking in later transport abstractions
...
- `RunWithHooks(...)` or an equivalently narrow lifecycle wrapper owns the run-phase ordering: once workspace creation has succeeded, it runs `before_run`, executes the run body only if `before_run` succeeds, and still invokes `after_run` in a `defer`/`finally`-equivalent on every exit path.
...
The orchestrator already has the cleanup intent in `CleanupWorkspace`. T07 should make that intent observable in the workspace API without inventing a second policy engine, and it should define the host-aware cleanup behavior that later orchestrator startup/runtime paths will call.
