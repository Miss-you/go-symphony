...
The first review round blocked the initial draft with two valid findings:

- runner admission cannot take only `preferredHost` if it is expected to enforce per-host capacity
- runner must not absorb workspace lifecycle CRUD policy that belongs in `internal/workspace`

This revised plan narrows the boundary:

- `internal/runner` owns transport execution and stateless host-choice logic
- `internal/orchestrator` remains the owner of mutable running-state counts and total/state concurrency gates
- `internal/workspace` keeps create/reuse/remove policy and calls runner only to execute commands on a selected host
...
T08 should make local and SSH command execution share one Go contract, without moving mutable runtime state out of the orchestrator and without moving workspace lifecycle policy out of `internal/workspace`.

## Source-Faithful Boundary

The Elixir implementation has these relevant responsibilities:
...
- `AgentRunner` and `Codex.AppServer` use the selected host and workspace path but do not make workspace lifecycle policy.

The Go boundary should mirror that split:
...
- adapt the existing `serviceDeps.admitRun(preferredHost)` seam to call `runner.HostSelection.Select` with host loads derived from orchestrator-owned running state, or leave the seam in place while adding package tests that prove the runner selector can preserve the Elixir host-choice rules.
...
### 4. Preserve orchestrator ownership

Avoid broad orchestrator rewrites.

If an orchestrator adapter is added, it should be private and shaped like:
...
The current `serviceDeps.admitRun` seam may remain until full runtime wiring in T14 if that keeps T08 narrow. The important T08 rule is that runner exposes a selector that can preserve the Elixir behavior without taking ownership of mutable state.
...
3. `go test ./internal/orchestrator/...`
   - proves runtime state ownership and host facts still compile/pass around the existing seams
4. `go test ./...`
...
- `internal/runner` has a tested stateless host selector that can preserve Elixir per-host capacity behavior from caller-supplied host loads
...
- `internal/orchestrator` remains the only owner of mutable running/retry/claim state
