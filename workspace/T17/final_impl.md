...
- When `server.port` is configured, `internal/cli.StartRuntime` starts an HTTP server that mounts this handler. CLI flag parsing and startup acknowledgement remain T18 scope.
...
- Add minimal `internal/cli` runtime server lifecycle wiring required to make configured web routes reachable.
