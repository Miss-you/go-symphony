...
`final_impl_v1.md` passed the required rubric review after one correction round.

Round-two review results:

- `review_3.md`: 98 / 100, no high-severity issues
- `review_4.md`: 93 / 100, no high-severity issues
- average: 95.5 / 100

Key review corrections accepted into this final plan:

- make failure-retry claim retention explicit so failure and continuation both hold the claim until retry revalidation resolves the item
- freeze retry lineage and backoff progression precisely enough to prevent divergent implementations
- freeze exact snapshot sort keys, aggregate totals semantics, and explicit acceptance checks
- clarify that continuation seeds a new retry lineage at attempt `1`, but capacity-blocked redispatch transitions into the failure backoff path

Acceptance decision:

- average score exceeds the `>= 80` threshold
- no round-two reviewer reported a remaining high-severity issue
- low-risk clarifications raised in round two are incorporated below
