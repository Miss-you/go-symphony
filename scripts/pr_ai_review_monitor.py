...
def is_ai_thread(thread: dict) -> bool:
...
def collect_findings(pull_requests: list[dict]) -> list[dict]:
    findings: list[dict] = []
    for pr in pull_requests:
        threads = []
        for thread in pr["reviewThreads"]["nodes"]:
            if thread["isResolved"] or thread["isOutdated"] or not is_ai_thread(thread):
                continue
...
def render_report(repo: str, scanned_count: int, findings: list[dict]) -> str:
    lines = [
    ...
    if not findings:
        lines.append("No unresolved Copilot or AI review threads found.")
        return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    findings = collect_findings(pull_requests)
    report = render_report(args.repo, len(pull_requests), findings)
