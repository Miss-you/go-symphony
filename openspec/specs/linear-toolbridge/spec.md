...
Define the Linear compatibility-shell ToolBridge contract for `go-symphony`, including the `linear_graphql` Codex dynamic tool, Symphony-compatible tool result shaping, Linear-specific write helpers, and the boundary that keeps provider writes out of core tracker/domain/orchestrator packages.
