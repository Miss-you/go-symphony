...
Define the Linear-specific read adapter contract for the Go compatibility shell. The adapter preserves Symphony's current Linear read behavior while implementing the provider-neutral `tracker.TrackerReader` interface and keeping tracker writes outside the core.
